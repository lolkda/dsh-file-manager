...
function profile() {
  ...
  return {
    ...
    ctx: {
      settings: {
        register(ns, schema) { settings.set(ns, schema()); },
        get(ns) { return settings.get(ns); },
      },
      storageDomain: {
        async open(spec) {
          lastSpec = spec;
...
test('profile storage restores explicit grants after the Host adapter is reopened', async t => {
  ...
  const first = await openProfileState(fixture.ctx);
  const manager = createManager(first.managerOptions);
  const grant = await manager.addRoot({ path: root });
  await first.close();
  const reopened = await openProfileState(fixture.ctx);
  ...
  const third = await openProfileState(fixture.ctx);
...
test('profile limits provide the approved defaults', async () => {
  const { openProfileState } = await stateModule();
  const fixture = profile();
  const state = await openProfileState(fixture.ctx);
  assert.equal(state.limits.maxTextBytes, 5 * 1024 * 1024);
  assert.equal(state.limits.maxFileBytes, 2 * 1024 ** 3);
  assert.equal(state.limits.maxTaskBytes, 10 * 1024 ** 3);
  assert.equal(state.limits.maxTaskEntries, 10000);
  assert.equal(state.limits.transferConcurrency, 2);
  await state.close();
});
