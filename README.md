...
当前适配版为 **0.4.0**，面向 DSH **0.1.7-rc.1**。本代次修改两处行为：上传确认使用无外圈描边的主题菜单并约束弹窗高度链，永久删除的准备与提交改为元数据清单、不再读取文件内容做哈希（其余强校验不变）；上一代次 0.3.0 加入 Client 内联编辑器与首批语言语法高亮（SPEC 的 R21），编辑器与文法仍随 `dist/client.js` 打包。**发布与安装状态不在仓库内声明**——本仓库不记录 registry 发布结果，也不记录安装到某个 DSH 实例的结果，公开状态以 [npm registry](https://www.npmjs.com/package/@lolkda/dsh-file-manager) 为准。开发期验证记录：类型检查、构建与 **683 项回归（0 失败 0 跳过）**，另含发布 workflow 门禁回归与 16 组隔离浏览器验收；真实完整页面的键盘/输入法与像素滚动仍未验收。下方保留旧版本验证记录。
...
- **文件操作**：新建文件/目录、重命名、永久删除。删除确认绑定服务端元数据清单、目录成员和确认期限，需要人工明确勾选并确认；准备和提交不读取文件内容。
...
- 列表和永久删除清单使用轻量元数据。永久删除准备及提交核对 `dev/ino/type/size/mtimeNs/ctimeNs` 与目录成员，不读取文件内容、不计算 SHA-256，也不受 `maxVerificationBytes` 限制；仍保留条目上限、确认期限及路径/根/链接保护，不再提供逐字节内容一致性证明。普通文件的显式版本读取、保存、覆盖、传输及跨卷移动删源继续使用内容强校验；弱元数据版本不能授权覆盖。
